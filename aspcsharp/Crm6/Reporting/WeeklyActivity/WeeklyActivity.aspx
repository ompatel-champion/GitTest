<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="WeeklyActivity.aspx.cs" Inherits="Crm6.Reporting.WeeklyActivity.WeeklyActivity" %>

<%@ Register Src="~/_usercontrols/nav.ascx" TagPrefix="uc1" TagName="nav" %>
<%@ Register Src="~/_usercontrols/nav-mobile.ascx" TagPrefix="uc1" TagName="navmobile" %>
<%@ Register Src="~/_usercontrols/CalendarEventAddEdit/CalendarEventAddEdit.ascx" TagPrefix="uc1" TagName="CalendarEventAddEdit" %>

<!DOCTYPE html>
<html>
<head runat="server">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
    <title>Weekly Activity Report</title> 
    <link href="weekly-activity-18-mar-2020.css" rel="stylesheet" />
     <!-- favicon -->
    <link href="data:image/png;base64," rel="icon" type="image/x-icon" />
</head>

<body class="addEditPage">
    <form runat="server">
        <div id="main">
            <%-- hidden values --%>
            <asp:Label CssClass="hide" ID="lblUserId" runat="server" Text="0"></asp:Label>
            <asp:Label CssClass="hide" ID="lblSubscriberId" runat="server" Text="0"></asp:Label>
            <asp:Label CssClass="hide" ID="lblUserIdGlobal" runat="server" Text="0"></asp:Label> 
            <asp:Label CssClass="hide" ID="lblUsername" runat="server" Text=""></asp:Label>

            <%--Load from Session--%>
            <asp:Label CssClass="hide" ID="lblDateFormat" runat="server" Text="dd MM, yyyy"></asp:Label>
            <asp:Label CssClass="hide" ID="lblDateFormatMask" runat="server" Text="0"></asp:Label>

            <!--navbar mobile-->
            <uc1:navmobile runat="server" />

            <div class="container-fluid">
                <div class="row">

                    <!--.page-container-->
                    <div class="page-container reportPage">

                        <!--.page-content-->
                        <div class="page-content">

                            <!--navbar desktop-->
                            <uc1:nav runat="server" ID="navSidebar" />

                            <!- #content-->
                            <div id="content" class="animated fadeIn">

                                <!--.top-header-->
                                <header class="top-header globalHead">
                                    <h1 class="page-title">Weekly Activities</h1>
                                </header>

                                <%-- content --%>
                                <!--Page Content-->
                                <div class="wrapper  br-fields">

                                    <div class="ibox MB20 MT15">
                                        <div class="ibox-content PB0 PT10">
                                            <%-- activities by date range report --%>

                                            <div id="divReport">

                                                <%-- events report filter --%>
                                                <div id="divReportFilter" class="hide">

                                                    <%--Location dropdown--%>

                
                                                    <div class="row MT10">
                                                        <div class="col-lg-3">
                                                            <div class="form-group filled">
                                                                <label class="inputLabel">Year</label>
                                                                <asp:DropDownList runat="server" ID="ddlYear" CssClass="form-control">
                                                                </asp:DropDownList>
                                                            </div>
                                                        </div> 
                                                        <div class="col-lg-3">
                                                              <div class="form-group filled">
                                                                <label class="inputLabel">Week</label>
                                                                <asp:DropDownList runat="server" ID="ddlWeekOfYear" CssClass="inline form-control">
                                                                </asp:DropDownList>
                                                            </div>
                                                        </div>

                                                         <%--Checkboxes to select Calendar Events, Tasks and Notes--%>
                                                        <div class="col-lg-6">
                                                            <div class="form-group filter-check-boxes">
                                                                <span class="MR20">
                                                                    <asp:CheckBox CssClass="i-checks" runat="server" Checked="true" ID="chkCalendarEvents"></asp:CheckBox>
                                                                    Events
                                                                </span>
                                                                <span class="MR20">
                                                                    <asp:CheckBox CssClass="i-checks" runat="server" Checked="true" ID="chkTasks"></asp:CheckBox>
                                                                    Tasks
                                                                </span>
                                                                <span class="MR20">
                                                                    <asp:CheckBox CssClass="i-checks" runat="server" Checked="true" ID="chkNotes"></asp:CheckBox>
                                                                    Notes
                                                                </span>
                                                            </div>
                                                        </div>
                                                    </div>



                                                     <div class="row ">

                                                        <div class="col-md-6">
                                                            <div class="form-group filled" runat="server" id="divCountries">
                                                                <label class="inputLabel language-entry">Countries</label>
                                                                <asp:DropDownList runat="server" ID="ddlCountry" CssClass="form-control" multiple="multiple"></asp:DropDownList>
                                                            </div>
                                                        </div>
                                                        <div class="col-md-6">
                                                            <div class="form-group filled" runat="server" id="divLocations">
                                                                <label class="inputLabel language-entry">Locations</label>
                                                                <asp:DropDownList runat="server" ID="ddlLocations" CssClass="form-control" multiple="multiple"></asp:DropDownList>
                                                            </div>
                                                        </div>
                                                        <div class="col-md-12" id="userContainer">
                                                            <div class="form-group filled" runat="server" id="divUsers">
                                                                <label class="inputLabel language-entry">Users</label>
                                                                <asp:DropDownList runat="server" ID="ddlUsers" CssClass="form-control" multiple="multiple"></asp:DropDownList>
                                                            </div>
                                                        </div>
                                                    </div>


                                                    <div class="row ">
                                                        <div class="col-md-8">
                                                         <div class="form-group filled">
                                                                <label class="inputLabel">Categories</label>
                                                                <asp:DropDownList runat="server" ID="ddlCategories" CssClass="form-control" multiple="multiple">
                                                                </asp:DropDownList>
                                                            </div>
                                                        </div> 
                                                        <div class="col-md-4">
                                                           <div class="form-group filled">
                                                                <label class="inputLabel">Campaign</label>
                                                                <asp:DropDownList runat="server" ID="ddlCampaigns" CssClass="form-control">
                                                                </asp:DropDownList>
                                                            </div>
                                                        </div>
                                                    </div>

                                                </div>

                                                <div class="clearfix"></div>
                                                <div class="row footer-actions">
                                                    <div class="col-md-5 col-sm-5">
                                                        <h4 class="font-bold total-record-count"></h4>
                                                    </div>
                                                    <div class="col-md-7 col-sm-7">
                                                        <div id="divToolbar">
                                                            <button id="btnRunReport" class="primary-btn language-entry">Run Report</button>
                                                            <a id="btnExcel" href="javascript:void(0)" class="primary-btn language-entry hide">Excel</a> 
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>

                                    <div class="ibox MB50 hide" id="divReportContent">
                                        <div class="ibox-content P0" style="height: 500px">

                                            <%-- deals report header --%>
                                            <table class="table table-hover" id="tblActivityReport">
                                                <thead>
                                                    <tr>
                                                        <th class=""><span class="language-entry">User</span></th>
                                                        <th class=""><span class="language-entry">Date</span></th>
                                                        <th class=""><span class="language-entry">Type</span></th>
                                                        <th class=""><span class="language-entry">Subject</span></th>
                                                        <th class=""><span class="language-entry">Description</span> </th>
                                                        <th class=""><span class="language-entry">Deal</span></th>
                                                        <th class=""><span class="language-entry">Companies</span></th>
                                                        <th class=""><span class="language-entry">Contacts</span></th>
                                                        <th class=""><span class="language-entry">Created</span></th>
                                                        <th class=""><span class="language-entry">Last Updated</span></th>
                                                        <th class="W100"><span class="language-entry"></span></th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    <%-- render events rows --%>
                                                </tbody>

                                                <tfoot>
                                                    <%-- render events footer --%>
                                                </tfoot>
                                            </table>

                                        </div>

                                        <%-- no deals --%>
                                        <div id="divNoItems" class="hide">
                                            <div class="alert alert-warning text-center PT50">
                                                <i class="fa fa-4x fa-building text-warning m-b-md"></i>
                                                <p>
                                                    <label class="language-entry">No Activities Found</label>
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>

                            <%-- CALENDAR EVENT ADD/EDIT --%>
                            <uc1:CalendarEventAddEdit runat="server" ID="CalendarEventAddEdit" />

                        </div>
                    </div>
                </div>
            </div>
        </div>

        <div class="modal inmodal" id="addTaskDialog" role="dialog" style="display: none;" aria-hidden="true">
            <div class="modal-dialog modal-lg">
                <div class="modal-content animated fadeIn">
                    <div class="modal-header">
                        <h4 class="modal-title">New Task</h4>
                        <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">x</span></button>
                    </div>
                    <div class="modal-body white-bg" style="min-height: 300px;">
                        <div class="ibox-content br-fields">
                            <div class="form-group">
                                <label class="inputLabel">Task name</label>
                                <input id="txtTaskTitle" class="task-name required" type="text" name="task_name" placeholder="">
                            </div>
                            <div class="form-group">
                                <label class="inputLabel">Add description</label>
                                <textarea id="txtTaskDescription" class="task-descp required" placeholder=""></textarea>
                            </div>
                            <div class="task-fields">
                                <div class="row">
                                    <div class="col-md-6 col-left-box">
                                        <div class="form-group">
                                            <label class="inputLabel">Due Date</label>
                                            <input type="text" id="txtDueDate" name="due_date" class="required" placeholder="" />
                                        </div>
                                    </div>
                                    <div class="col-md-6 col-right-box">
                                        <div class="form-group">
                                            <label class="inputLabel">Assign To<span class="req">*</span></label>
                                            <asp:DropDownList ID="ddlTaskSalesReps" CssClass="custom-select2" runat="server"></asp:DropDownList>
                                        </div>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-md-6 col-left-box">
                                        <div class="form-group">
                                            <label class="inputLabel">Related Deal</label>
                                            <asp:DropDownList ID="ddlTaskRelatedDeal" CssClass="custom-select2" runat="server"></asp:DropDownList>
                                        </div>
                                    </div>
                                    <div class="col-md-6 col-right-box">
                                        <div class="form-grou">
                                            <label class="inputLabel">Related Contacts</label>
                                            <asp:DropDownList ID="ddlTaskRelatedContacts" CssClass="custom-select2" runat="server"></asp:DropDownList>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>

                    <div class="modal-footer footer-action">
                        <button type="button" class="secondary-btn" data-dismiss="modal">Close</button>
                        <div class="btn-wrp" id="task-add-actions">
                            <a class="primary-btn" id="btnAddTask">Add Task</a>
                        </div>
                    </div>
                </div>
            </div>
        </div>

    </form>
</body>
<script src="weekly-activity-21-mar-2020.js"></script>

</html>

